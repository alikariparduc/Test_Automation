...
public class RejectTransferDBMethods extends DataBaseMethods {
    public static void controlRejectTransferOperationsInTRANSFERDatabase_V2_TRANSACTION_Table(Statement st, String sessionID) throws SQLException {
        ...
        try{
            ...
            if(rs.next()) {
                if(rs.getInt("IS_ACTIVE") == 1 &&
                   rs.getString("OPERATION_TYPE").equals("REJECT") &&
    ...
    public static void controlRejectTransferOperationsInSTOCKDatabase_V2_INVENTORY_TRANSFER_PENDING_Table(Statement st, String sessionID) throws SQLException {

        try{
            //Session ID için kaydı getir
            ResultSet rs = st.executeQuery("SELECT * FROM V2_INVENTORY_TRANSFER_PENDING WHERE SESSION_ID = '"+sessionID+"'");

            //Transfer reddedildiği edildiği için kayıt gelmemesi gerekiyor.
            if(!rs.next()) {
